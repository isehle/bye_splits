...
data_dir = params.base_kw['BasePath']
...
def get_keys(init_files, pars):
    ...
    #file_name = start+re.split('gen_cl3d_tc',init_files['photons'][0])[1]
...
def display_color(coef, eta_range, normby):
    df_by_particle = get_dfs(input_files, coef)
    ...
    glob_effs = pd.DataFrame({'photons': np.mean(phot_effs[1:]),
                    'pions': np.mean(pion_effs[1:])
                    }, index=[0])
...
def global_effs(eta_range, normby, file="global_eff.hdf5"):
    ...
    try:
        fig.add_trace(go.Scatter(x=coefs, y=effs_by_coef['photons'], name='photons'), row=1, col=1)
        fig.add_trace(go.Scatter(x=coefs, y=effs_by_coef['pions'], name='pions'), row=1, col=2)
    except:
        fig.add_trace(go.Scatter(x=coefs, y=effs_by_coef['Photon'], name='photons'), row=1, col=1)
        fig.add_trace(go.Scatter(x=coefs, y=effs_by_coef['Pion'], name='pions'), row=1, col=2)
